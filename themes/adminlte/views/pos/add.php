                    ...
                    <div class="tab-content">
                        ...
                        <div role="tabpanel" class="tab-pane" id="tab-cellular_plans">
                            <?php if($plans): foreach($plans as $plan): ?>
                                <button data-type="plans" value="<?php echo $plan->id; ?>" class="btn-pr btn btn-default"><span><?php echo character_limiter($plan->name , 40)?></span></button>
                            <?php endforeach; endif;?>
                        </div>
                ...
                <div class="form-group">
                    <div class="input-group">
                        ...
                        <select id="client_name" name="client_name" class="client_name form-control" style="width: 100%" required>
                            <option ><?php echo lang('Please Select A Client Or Add A New One');?></option>
                            <option value="-1" selected <?php echo $this->input->get('customer') && $this->input->get('customer') == -1 ? 'selected' : ''; ?>><?php echo lang('walk_in');?></option>
                            <?php 
                            ...
                            <div class="form-group">
                                <select required id="assigned_to" name="assigned_to" class="form-control" style="width: 100%">
                                    <?php
                                        ...
                                        echo '<option value="'.$user->id.'" '.($user->id == $this->session->userdata('user_id') ? 'selected' : ''). '>'.$user->first_name.' '.$user->last_name.'</option>';
                                        endforeach;
                                    ?>
                                </select>
                            </div>
...
<div class="modal modal-default-filled fade " id="prModal" data-backdrop="false">
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header">
        ...
        <form class="form-horizontal" id="serial_form" role="form">
            ...
            <div id="id-div"></div>
            <div class="form-group">
                <div id="pserial_number-div">
                    <div class="form-group">
                        <label class="col-sm-4 control-label"><?php echo lang('Serial Number');?>: </label>
                        <div class="col-sm-8">
                            <input value="" autocomplete="off" id="pserial_number" name="pserial_number" class="form-control" />
                        </div>
                    </div>
                    <div class="form-group">
                        <label class="col-sm-4 control-label"><?php echo lang('Purchase Type');?>: </label>
                        <div class="col-sm-8">
                            <select name="purchase_type" id="purchase_type" class="form-control">
                                <option value="1"><?php echo lang('Outright Purchase');?></option>
                                <option value="2"><?php echo lang('Activation Purchase');?></option>
                            </select>
                        </div>
                    </div>
    ...
    <div class="modal-footer">
        <button class="btn btn-danger" data-dismiss="modal"><?php echo lang('Cancel');?></button>
...
jQuery(document).ready( function($) {
    ...
    $("#basic-wizard").on( "submit", function( event ) {
        ...
        if(item.options !== null && item.options !== false) {
            ...
            item.price = cost;
            item.option = $('#vpoption').val() ? $('#vpoption').val() : '',
            item.option_name = name,
            item.option_selected = true;
            item.phone_number = $('#v_phone_number').inputmask('unmaskedvalue');
            item.set_reminder = vset_reminder;
        }
    ...
    function loadItems(edit_items = true) {
        if (localStorage.getItem('positems')) {
            ...
            $.each(items, function () {
                ...
                var activation_price = parseFloat(this.row.activation_price);
                ...
                $.each(this.options, function (x, y) {
                    if(y.id == item_option) {
                        ...
                        if ((type == 'new_phone' || type == 'used_phone') && (parseFloat(price) == parseFloat(activation_price))) {
	...
	function processTransaction(responseData) {
        ...
        $(document).on('click', '#submit-sale', function () {
            ...
            if (total_paid == 0 || total_paid < grand_total) {
                // bootbox.alert("<?php echo lang('paid_l_t_payable');?>");
...
// 
    ...
    function add_pos_item(item, edit_item=true) {
        ...
        if (item.type == 'other' && parseInt(item.row.keep_stock) == 0) {
            ...
            if (current_type == 'drepairs' || current_type == 'crepairs') {
               ...
               if(item.row.comment !== ''){
               bootbox.alert(item.row.comment);
               }
            }else if (current_type == 'plans') {
        ...
        // open modal. do things and than add
        selectedItem = item;
        item_option = item.option;
        row_no = item.row_id;
        type = current_type;
        price = 0;
        $.each(item.options, function (x, y) {
            if(y.id == item_option) {
                if (type !== 'repair') {
                    cost = y.cost;
                }
        ...
        $('#options_div_').hide();
        if (item.variants == true && item.option_selected == false && item.type == "plans") {
            $('#vid-div').html('<input type="hidden" name="prvow_id" id="prvow_id" value="'+row_no+'"><input type="hidden" name="prvtype" id="prvtype" value="'+item.row.type+'">');
        ...
        }else if ((item.variants == true && item.option_selected == false && item.type !== "plans") || (!item.serialed) || !variable_price_picked || ((item.type == 'new_phone' || item.type == 'used_phone' ) && !item.purchase_type)) {
            var opt = '<p style="margin: 12px 0 0 0;">n/a</p>';
            $('#options_div_').hide();
            $('#hidden_sap').hide();
            $('#purchase_type').val(1).trigger('change');
            ...
            if(item.options !== null && item.options !== false) {
                    ...
                    if(o == 1) {
                        ...
                        if (item.type == 'new_phone' || item.type == 'used_phone') {
                            code = y.name;
                        }
                        if (item.type == 'plans') {
                            activation_spiff = y.activation_spiff
            ...
            if (item.is_serialized) {
                $('#pserial_number').val((item.serial_number?item.serial_number:''));
                $('#pserial_number').data('id', item.product_id);
                $('#pserial_number').data('type', type);
            }else{
                $('#pserial_number').parent().parent().hide();
            }
            ...
            if (type == 'new_phone' || type == 'used_phone') {
                ...
                $('#purchase_type').parent().parent().hide();
            }

            $('#id-div').html('<input type="hidden" name="prow_id" id="prow_id" value="'+row_no+'"><input type="hidden" name="prtype" id="prtype" value="'+item.row.type+'">');
            var vprice = '';
            ...
            if ((!item.refund_item) && item.type !== 'cp' && item.type !== 'plans') {
    ...
    async function addbyTypeAndID(type, code) {
...
$("#serial_form").on( "submit", function( event ) {
    ...
    if(item.options !== null && item.options !== false) {
        ...
        item.option = $('#poption').val() ? $('#poption').val() : '',
        item.option_name = name,
        item.option_selected = true;
    }
    if (item.row.type == 'other' && parseInt(item.row.variable_price) == 1) {
        ...
        if (pdescription) {
            item.item_details = pdescription;
        }
    ...
    if(item.is_serialized) {
        ...
        request = $.ajax({
            ...
            success: function(data) { 
                if (data) {
                    item.serial_number =  serial?serial:'';
                    item.serialed =  true;
                }else{
                    $('#prModal').appendTo('body').modal({backdrop: 'static', keyboard: false});
    ...
    if (document.getElementById('purchase_type')) {
        ...
        if(purchase_type == 2) {
            ...
            item.price = parseFloat(item.row.activation_price);
            item.row.discount_type = parseInt(item.row.discount_type2);
            item.row.max_discount = parseInt(item.row.max_discount2);
            ...
            // addbyTypeAndID('plans', sap_pplan);
  ...
  /* ------------------------------
    ...
    $(document).on('click', '#addItemManually', function(e) {
        var mid = new Date().getTime(),
            mcode = $('#mcode').val() ? $('#mcode').val() : jQuery.now(),
            mname = $('#mname').val(),
